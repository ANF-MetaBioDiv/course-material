...
We import the ASV table, the taxonomic assignment results and the sequences from the text files we exported in the [dada2 practical](/practicals/preprocessing_dada2.html#export-asv-table).
...
Use the `phyloseq::phyloseq()` function to create a phyloseq object. A phyloseq object is usualy composed by an ASV table, a taxonomy table and a table describing the samples. You can also add ASV sequences and a [phylogenetic tree](#add-phylogenetic-tree-optional)
...
Other approaches to reconstruct a phylogenetic tree exist. If you want to try them with `phangorn`, have a look [here](https://cran.r-project.org/web/packages/phangorn/vignettes/Trees.html){target="_blank"}
...
In that case, it is fine. You will still be able to perform most of the analyses introduced in the [alpha](/practicals/alpha_diversity.html) and [beta diversity](/practicals/beta_diversity.html) practicals.
