...
To do so, visit the ([ANF-metabarcoding](https://github.com/ANF-MetaBioDiv/course-material)) repository on github and download it as a zip file.
...
The "classic" way to import functions is to use `source()` with the name of the R script to source.

Instead, we use `devtools::load_all()`. This function will source all the scripts from the folder `R/`
...
`|>`: R "pipe". It allows you to chain functions, avoiding intermediate variables and nested parenthesis.
It basically transfers the output of the left expression to the input of the right expression.
You need R \> 4.1 to use this pipe, otherwise use `%>%` from [`magrittr`](https://magrittr.tidyverse.org/).
...
```{r}
seq_zoom <- Biostrings::readDNAStringSet(
  fnFs[1],
  format = "fastq",
  nrec = 10
)
...
If you have to deal with mix-orientated paired-end reads, you may find inspiration [here](https://gitlab.sb-roscoff.fr/nhenry/abims-metabarcoding-pipeline-dada2)
